...
impl<'a> ListBuilder<'a> {
    ...
    pub fn send(&mut self) -> Result<RemindersListResponse> {
        let payload = self.client.send(format!("{}?{:?}", "reminders.list", &self.params))?;
        payload.to_type::<RemindersListResponse>().or_else(|_| Err(self.client.read_error(&payload)))
    }
}

impl Client {
    /// A shorthand method to call the Slack API directly
    pub fn reminders_list<>(&mut self, ) -> ListBuilder {
