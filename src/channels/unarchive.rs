...
impl<'a> UnarchiveBuilder<'a> {
    ...
    pub fn send(&mut self) -> Result<ChannelsUnarchiveResponse> {
        let payload = self.client.send(format!("{}?{:?}", "channels.unarchive", &self.params))?;
        payload.to_type::<ChannelsUnarchiveResponse>().or_else(|_| Err(self.client.read_error(&payload)))
    }
}

impl Client {
    /// A shorthand method to call the Slack API directly
    pub fn channels_unarchive<S: Into<String>>(&mut self, channel: S) -> UnarchiveBuilder {
