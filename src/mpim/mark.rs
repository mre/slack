...
impl<'a> MarkBuilder<'a> {
    ...
    pub fn send(&mut self) -> Result<MpimMarkResponse> {
        let payload = self.client.send(format!("{}?{:?}", "mpim.mark", &self.params))?;
        payload.to_type::<MpimMarkResponse>().or_else(|_| Err(self.client.read_error(&payload)))
    }
}

impl Client {
    /// A shorthand method to call the Slack API directly
    pub fn mpim_mark<S: Into<String>>(&mut self, channel: S, ts: f64) -> MarkBuilder {
