...
impl<'a> ArchiveBuilder<'a> {
    ...
    pub fn send(&mut self) -> Result<ChannelsArchiveResponse> {
        let payload = self.client.send(format!("{}?{:?}", "channels.archive", &self.params))?;
        payload.to_type::<ChannelsArchiveResponse>().or_else(|_| Err(self.client.read_error(&payload)))
    }
}

impl Client {
    /// A shorthand method to call the Slack API directly
    pub fn channels_archive<S: Into<String>>(&mut self, channel: S) -> ArchiveBuilder {
