...
impl<'a> InfoBuilder<'a> {
    ...
    pub fn send(&mut self) -> Result<ChannelsInfoResponse> {
        let payload = self.client.send(format!("{}?{:?}", "channels.info", &self.params))?;
        payload.to_type::<ChannelsInfoResponse>().or_else(|_| Err(self.client.read_error(&payload)))
    }
}

impl Client {
    /// A shorthand method to call the Slack API directly
    pub fn channels_info<S: Into<String>>(&mut self, channel: S) -> InfoBuilder {
