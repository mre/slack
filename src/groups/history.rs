...
/// Include unread_count_display in the output?.
    ...
    pub fn send(&mut self) -> Result<GroupsHistoryResponse> {
        let payload = self.client.send(format!("{}?{:?}", "groups.history", &self.params))?;
        payload.to_type::<GroupsHistoryResponse>().or_else(|_| Err(self.client.read_error(&payload)))
    }
}

impl Client {
    /// A shorthand method to call the Slack API directly
    pub fn groups_history<S: Into<String>>(&mut self, channel: S) -> HistoryBuilder {
