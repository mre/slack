...
impl<'a> CloseBuilder<'a> {
    ...
    pub fn send(&mut self) -> Result<GroupsCloseResponse> {
        let payload = self.client.send(format!("{}?{:?}", "groups.close", &self.params))?;
        payload.to_type::<GroupsCloseResponse>().or_else(|_| Err(self.client.read_error(&payload)))
    }
}

impl Client {
    /// A shorthand method to call the Slack API directly
    pub fn groups_close<S: Into<String>>(&mut self, channel: S) -> CloseBuilder {
