...
impl<'a> LeaveBuilder<'a> {
    ...
    pub fn send(&mut self) -> Result<GroupsLeaveResponse> {
        let payload = self.client.send(format!("{}?{:?}", "groups.leave", &self.params))?;
        payload.to_type::<GroupsLeaveResponse>().or_else(|_| Err(self.client.read_error(&payload)))
    }
}

impl Client {
    /// A shorthand method to call the Slack API directly
    pub fn groups_leave<S: Into<String>>(&mut self, channel: S) -> LeaveBuilder {
