...
impl<'a> MarkBuilder<'a> {
    ...
    pub fn send(&mut self) -> Result<ImMarkResponse> {
        let payload = self.client.send(format!("{}?{:?}", "im.mark", &self.params))?;
        payload.to_type::<ImMarkResponse>().or_else(|_| Err(self.client.read_error(&payload)))
    }
}

impl Client {
    /// A shorthand method to call the Slack API directly
    pub fn im_mark<S: Into<String>>(&mut self, channel: S, ts: f64) -> MarkBuilder {
