...
impl<'a> CloseBuilder<'a> {
    ...
    pub fn send(&mut self) -> Result<ImCloseResponse> {
        let payload = self.client.send(format!("{}?{:?}", "im.close", &self.params))?;
        payload.to_type::<ImCloseResponse>().or_else(|_| Err(self.client.read_error(&payload)))
    }
}

impl Client {
    /// A shorthand method to call the Slack API directly
    pub fn im_close<S: Into<String>>(&mut self, channel: S) -> CloseBuilder {
