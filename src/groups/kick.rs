...
impl<'a> KickBuilder<'a> {
    ...
    pub fn send(&mut self) -> Result<GroupsKickResponse> {
        let payload = self.client.send(format!("{}?{:?}", "groups.kick", &self.params))?;
        payload.to_type::<GroupsKickResponse>().or_else(|_| Err(self.client.read_error(&payload)))
    }
}

impl Client {
    /// A shorthand method to call the Slack API directly
    pub fn groups_kick<S: Into<String>>(&mut self, channel: S, user: S) -> KickBuilder {
