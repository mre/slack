...
impl<'a> CreateBuilder<'a> {
    ...
    pub fn send(&mut self) -> Result<GroupsCreateResponse> {
        let payload = self.client.send(format!("{}?{:?}", "groups.create", &self.params))?;
        payload.to_type::<GroupsCreateResponse>().or_else(|_| Err(self.client.read_error(&payload)))
    }
}

impl Client {
    /// A shorthand method to call the Slack API directly
    pub fn groups_create<S: Into<String>>(&mut self, name: S) -> CreateBuilder {
