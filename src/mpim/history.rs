...
/// Include unread_count_display in the output?.
    ...
    pub fn send(&mut self) -> Result<MpimHistoryResponse> {
        let payload = self.client.send(format!("{}?{:?}", "mpim.history", &self.params))?;
        payload.to_type::<MpimHistoryResponse>().or_else(|_| Err(self.client.read_error(&payload)))
    }
}

impl Client {
    /// A shorthand method to call the Slack API directly
    pub fn mpim_history<S: Into<String>>(&mut self, channel: S) -> HistoryBuilder {
