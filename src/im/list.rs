...
impl<'a> ListBuilder<'a> {
    ...
    pub fn send(&mut self) -> Result<ImListResponse> {
        let payload = self.client.send(format!("{}?{:?}", "im.list", &self.params))?;
        payload.to_type::<ImListResponse>().or_else(|_| Err(self.client.read_error(&payload)))
    }
}

impl Client {
    /// A shorthand method to call the Slack API directly
    pub fn im_list<>(&mut self, ) -> ListBuilder {
