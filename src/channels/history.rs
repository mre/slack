...
/// Include unread_count_display in the output?.
    ...
    pub fn send(&mut self) -> Result<ChannelsHistoryResponse> {
        let payload = self.client.send(format!("{}?{:?}", "channels.history", &self.params))?;
        payload.to_type::<ChannelsHistoryResponse>().or_else(|_| Err(self.client.read_error(&payload)))
    }
}

impl Client {
    /// A shorthand method to call the Slack API directly
    pub fn channels_history<S: Into<String>>(&mut self, channel: S) -> HistoryBuilder {
