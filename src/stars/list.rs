...
impl<'a> ListBuilder<'a> {
    ...
    pub fn send(&mut self) -> Result<StarsListResponse> {
        let payload = self.client.send(format!("{}?{:?}", "stars.list", &self.params))?;
        payload.to_type::<StarsListResponse>().or_else(|_| Err(self.client.read_error(&payload)))
    }
}

impl Client {
    /// A shorthand method to call the Slack API directly
    pub fn stars_list<>(&mut self, ) -> ListBuilder {
