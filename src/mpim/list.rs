...
impl<'a> ListBuilder<'a> {
    ...
    pub fn send(&mut self) -> Result<MpimListResponse> {
        let payload = self.client.send(format!("{}?{:?}", "mpim.list", &self.params))?;
        payload.to_type::<MpimListResponse>().or_else(|_| Err(self.client.read_error(&payload)))
    }
}

impl Client {
    /// A shorthand method to call the Slack API directly
    pub fn mpim_list<>(&mut self, ) -> ListBuilder {
