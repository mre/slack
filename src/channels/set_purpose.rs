...
impl<'a> SetPurposeBuilder<'a> {
    ...
    pub fn send(&mut self) -> Result<ChannelsSetPurposeResponse> {
        let payload = self.client.send(format!("{}?{:?}", "channels.setPurpose", &self.params))?;
        payload.to_type::<ChannelsSetPurposeResponse>().or_else(|_| Err(self.client.read_error(&payload)))
    }
}

impl Client {
    /// A shorthand method to call the Slack API directly
    pub fn channels_set_purpose<S: Into<String>>(&mut self, channel: S, purpose: S) -> SetPurposeBuilder {
